    ...
    <div v-if="isSignedIn">
      <v-card class="mx-auto" max-width="800" tile>
        <v-list-item three-line>
          <v-list-item-content>
            <v-list-item-title class="display-2">Liste des club</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-card>
      ...
      <div v-for="row in playerStatToShow" :key="row.id">
        ...
        <v-list-item two-line>
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="headline">{{ row.private_club }}</v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
        ...
        <v-list-item two-line>
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="headline">{{ row.is_admin }}</v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
      </div>
    </div>
...
export default {
  ...
  async created() {
    const clubs = await axios.get(
      "https://fama6831.odns.fr/dbcontrol/api/v1/Clubs/id" + this.id,
      {
        responseType: "json"
      }
    );
